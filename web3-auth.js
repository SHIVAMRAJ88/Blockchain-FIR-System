...
// 1. Missing imports add kiye gaye hain: getApps, getApp, aur get
...
// 2. App aur DB ko sahi se initialize karke export kiya gaya hai
export const app = getApps().length === 0 ? initializeApp(firebaseConfig) : getApp();
...
export async function loginWithMetaMask() {
    ...
    try {
        ...
        const userRef = ref(db, `users/${walletAddress}`); 
        const snapshot = await get(userRef); // Ab 'get' kaam karega kyunki upar import hai
...
async function handleTraditionalLogin() {
    ...
    if (!phone || !password || !selectedRole) {
        ...
        if (errorMsg) errorMsg.innerText = "Fill all fields and select role";
        return;
    }
        ...
        if (snapshot.exists()) {
            ...
            if (userData.password === password) {
                if (userData.role.toLowerCase() === selectedRole.toLowerCase()) {
                    ...
                    document.getElementById("error").innerText = `Access Denied: You are a ${userData.role}.`;
                }
            } else {
                document.getElementById("error").innerText = "Wrong Password!";
            }
